...
function mousePressed() {
	...
	//
	// For debug use !?!?! //
	// console.log("Mouse Pressed:",amouseX,amouseY);
}
...
Game.prototype.debug = function () {
	if(this.flags.debug){
		...
		text("c["+Math.floor(terrain.centerChunk.x)+','+Math.floor(terrain.centerChunk.y)+"]",10,70);
...
Terrain.prototype.update = function () {
	// determine the center chunk
	var cx = Math.floor(camera.x / this.chunkSize.x);
	var cy = Math.floor(camera.y / this.chunkSize.y);

	// if current center chunk then return
	if(createVector(cx,cy).equals(this.centerChunk)) return;
	// else:
	console.log('Updating Chunks');
	var centerChunk = createVector(cx, cy);
	this.centerChunk = centerChunk;
	// load missing chunks & unload unnessary chunks
	var cs = [
		p5.Vector.add(centerChunk, createVector( 0,  0)), // C
		p5.Vector.add(centerChunk, createVector(-1,  0)), // T
		p5.Vector.add(centerChunk, createVector(-1,  1)), // TR
		p5.Vector.add(centerChunk, createVector( 0,  1)), // R
		p5.Vector.add(centerChunk, createVector( 1,  1)), // BR
		p5.Vector.add(centerChunk, createVector( 1,  0)), // B
		p5.Vector.add(centerChunk, createVector( 1, -1)), // BL
		p5.Vector.add(centerChunk, createVector( 0, -1)), // L
		p5.Vector.add(centerChunk, createVector(-1, -1))  // TL
	];
	var to_rem = [];
	var no_gen = [];
	var to_gen = [];
	var pos, rem;
	for (var ci = this.chunks.length - 1; ci >= 0; ci--) {
		rem = true;
		...
		if(rem) to_rem.push(ci);
	}
	for (var i = cs.length - 1; i >= 0; i--) {
		if(no_gen.indexOf(i) == -1) to_gen.push(i);
	}
	...
	var chunk_pos;
	for (var i = to_gen.length - 1; i >= 0; i--) {
		chunk_pos = cs[to_gen[i]];
		this.chunks.push(this._generateChunk(chunk_pos));
	}

	console.log(to_rem, to_gen);
...
Terrain.prototype.drawDebug = function () {
	...
	var camOffset = createVector(camera.x, camera.y).mult(0.05);
	//
	rectMode(CORNER);
	stroke(200);
	var pos;
	for (var i = this.chunks.length - 1; i >= 0; i--) {
		pos = p5.Vector.add(p5.Vector.mult(this.chunks[i].pos, 0.05), offset).sub(camOffset);
		if(this.chunks[i].chunk_pos.equals(this.centerChunk)){fill(0,0,200,50);}else{noFill()}
		rect(pos.x, pos.y, this.chunkSize.x*0.05, this.chunkSize.y*0.05);
	}
	//
	rectMode(CENTER);
	noFill();
